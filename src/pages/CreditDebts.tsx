...
export function CreditDebts() {
  ...
  const debtGroups = useMemo(() => {
    ...
    rows.forEach((row) => {
      ...
      if (String(row.payment_status || '') !== 'Dette Totale') return;
    ...
    return Array.from(grouped.values()).sort((a, b) => b.outstandingAmount - a.outstandingAmount);
  }, [rows]);
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {debtGroups.map((group) => (
            <article key={group.key} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600">
                <span className="inline-flex items-center gap-1"><CreditCard className="h-3.5 w-3.5" /> {group.debtCount} crédits ouverts</span>
                <span className="inline-flex items-center gap-1"><Wallet className="h-3.5 w-3.5" /> Depuis {new Date(group.oldestAt).toLocaleDateString('fr-FR')}</span>
              </div>
              <Button
